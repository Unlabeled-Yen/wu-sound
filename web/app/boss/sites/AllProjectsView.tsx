...
// 「全部專案」分頁——原本 sites/page.tsx 的主檔維護畫面整批搬過來,邏輯/
// actions.ts 完全不動(design_handoff_wu_sound/15-placebook.md commit 1:
// 純搬家,不改任何 server action)。地點簿(PlacebookView)接手當首頁,
// 這裡專門處理新增/改名/改類別/改客戶/啟用停用。
...
export async function AllProjectsView({ sb }: { sb: ReturnType<typeof getSupabaseAdmin> }) {
  ...
  return (
    <div>
      <p className="text-[13px] mb-4" style={{ color: 'var(--nm-text-secondary)' }}>
        共 {sites.length} 個專案。停用後不會出現在新的下拉選單,但歷史紀錄保留。一案一性質:同客戶既有安裝又辦活動,請開兩個案子。
      </p>
