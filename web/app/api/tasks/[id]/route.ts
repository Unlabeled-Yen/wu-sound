...
export async function PATCH(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const cur = await supabase.from('tasks').select('status, blocked_since').eq('id', id).maybeSingle();
  ...
  if (body.status !== undefined) {
    ...
    const blockedOn = body.blocked_on != null ? String(body.blocked_on).trim() : null;
    const err = validateTaskMove({ to_status: toStatus, blocked_on: blockedOn });
    if (err) return NextResponse.json({ error: err }, { status: 400 });

    patch.status = toStatus;
    patch.blocked_on = toStatus === 'blocked' ? blockedOn : null;
    // blocked_since:移入 blocked 時起算,移出時清空,重複停在 blocked 不重算起點。
    ...
    // completed_at:移入 done 時起算(封存倒數的起點),移出時清空。
    if (toStatus === 'done' && cur.data.status !== 'done') {
