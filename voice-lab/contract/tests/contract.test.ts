 ...
 * 工具面契約測試 v1.0
 *
 * 用法:BASE_URL=http://localhost:8787 API_KEY=dev-key npm test
...
describe('通用', () => {
  it('無 API key 一律 401', async () => {
    const res = await fetch(`${BASE}/tools/search_projects`, {
      ...
      body: JSON.stringify({ query: '王' }),
    });
    expect(res.status).toBe(401);
  });

  it('回應帶契約版本 1.x', async () => {
    const { version } = await call('search_projects', { query: '王' });
...
describe('讀取工具', () => {
  it('search_projects 候選 ≤ 5 且含必要欄位', async () => {
    const { status, json } = await call('search_projects', { query: '王' });
  ...
  it('list_tasks ≤ 10 筆且帶 total', async () => {
    const { json: s } = await call('search_projects', { query: '王' });
...
describe('兩階段寫入 — token 由伺服器簽發', () => {
  async function firstProjectId(): Promise<string> {
    const { json } = await call('search_projects', { query: '王' });
