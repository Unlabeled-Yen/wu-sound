...
- [x] Agent runtime + 工具轉發完成(6 個 wu 工具 + 追問/回覆/拒絕 + 民生救難共 11 個)
- [x] 對話狀態機四態 UI 正確顯示
- [x] 8 個腳本化對話案例手動跑過:**6 通過 / 1 部分(案例 6 日期)/ 1 未過(案例 8 沿用 id)**,
      詳見 [lab2-conversation-cases.md](./lab2-conversation-cases.md);兩個未過項的判斷見上方「已知技術債」
- [x] `tsc --noEmit` + `npm run build` 乾淨,4081 個單元測試綠
- [x] 未改動 Lab 1 契約與轉接層任何程式碼

**Lab 2 於 2026-08-14 收斂**(Yen 確認先不修日期那題)。仍未做、需要 Yen 才能進行的:
Anthropic 對照組(`.env.local` 的 `ANTHROPIC_API_KEY` 目前是空的,跑的是 Kimi)。
...
## 已知技術債:日期值仍由模型產生(2026-08-14 Yen 確認先不修)

**問題**:`propose_create_task` 的 `due_date` 參數讓模型自己填 `YYYY-MM-DD`,
所以「下週三」要由它換算。實測 Kimi 約一半機率算錯一天(8/14 週五講下週三,填成 8/20 週四)。

**這是設計不一致,不是模型不夠好**。整個 Lab 2 的原則是「把模型能出錯的能力拿掉」——
它沒有寫入工具、沒有機會判斷使用者是否同意、不能憑印象講天氣、急救內容寫死。
日期是**唯一**留給它自己算的欄位,而且我們為它加過兩次 prompt 規則(自我覆核、直接發提案),
那正是本專案一路認定不可靠的做法。換 Anthropic 只會降低錯誤率,不會補起這個結構性的洞。

**現有防線**:確認卡片的期限欄由 runtime 算出星期幾一起顯示(`2026-08-20(週四)`),
使用者講「下週三」卻看到「週四」就會發現。錯誤沒有被消滅,但不再隱形——
而且是擋在按下確認之前。這是最後一道防線,目前也是唯一一道。

**該怎麼修(想清楚了,只是還沒做)**:

1. 用 `due_phrase`(原話,例如「下週三」)取代 `due_date`,模型連填日期的能力都沒有
2. runtime 解析;**解不出唯一答案就回 AMBIGUOUS,強制走追問**,不准挑一個最像的
3. 卡片的星期幾保留當最後一道

第 2 點的重點不是「解析器比模型準」,而是**解析器錯了是可重現、可寫測試、修一次就不再犯的 bug;
模型錯了是機率,只能一直重測**。

**解析器也解不掉、必須走追問的情況**(不要假裝解得掉):

- 「下週三」而今天正好是星期三 → 台灣人有人指 6 天後、有人指今天,這是真歧義
- 「月底前」→ 那是範圍不是日期,硬轉成 8/31 等於替使用者做了他沒做的決定
- 「等水電做完之後」→ 相對於事件而非日曆
